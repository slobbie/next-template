...
const SettingCertified = () => {
  ...
  return (
    ...
    >
      <Space bottom={40} />
      <TypoCommon typographyType="t5">비밀 번호를 설정해주세요</TypoCommon>
        ...
        {arr.map((item, i) => {
          return (
            <div key={i} className={completeStyle.numItem}>
              <TypoCommon typographyType="t4">{item}</TypoCommon>
            </div>
          )
        })}
        <div className={completeStyle.numItem}>
          <TypoCommon typographyType="t4">9</TypoCommon>
        </div>
        <div className={completeStyle.numItem}>
          <Image
            className={completeStyle.ArrowIcon}
            src={arrowIcon}
            alt="지우기 버튼"
          />
        </div>
