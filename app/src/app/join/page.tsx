...
const Join = () => {
  ...
  const keyDownCallback = (
    pId: string,
    pRef?: React.RefObject<HTMLInputElement> | null,
  ) => {
    setTrigger((prev) => {
      return {
        ...prev,
  ...
  return (
    <>
      ...
      >
        <Space bottom={40} />
        <TypoCommon typographyType="t5" textAlign="left">
          {title}
        </TypoCommon>
        <Space bottom={60} />
        {trigger.mbrNm && (
        ...
        {errorLog.length > 0 && (
          <div className={styles.errorLogBox}>
            <TypoCommon typographyType="st2" textAlign="left" color="red500">
              {errorLog}
            </TypoCommon>
            <Space top={10} />
          </div>
